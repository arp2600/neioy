...
def _server_connect_wrapper(server, connect_func, ip_address, port, **kwargs):
    assert isinstance(port, int)
    connect_func(server, ip_address=ip_address, port=port, **kwargs)

    from icecream import ic
    ic(server._maximum_logins)
    # Add the default group for this client
    dg = server.default_group
    ic(dg)
    server._node_children[dg.id_] = []
